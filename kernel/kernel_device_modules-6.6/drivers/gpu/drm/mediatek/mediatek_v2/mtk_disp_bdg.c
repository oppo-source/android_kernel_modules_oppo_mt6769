...
static void bdg_config_init(struct device_node *node)
{
	...
	if (!ret) {
		...
		bdg_mm_clk = MM_CLK;
		DDPINFO("%s: not find bdg-mm-clk, set as: %d!", __func__, bdg_mm_clk);
	}

	ret = of_property_read_u32(node, "bdg-rx-v12", &bdg_rx_v12);
	if (!ret) {
		DDPINFO("%s: read bdg-rx-v12:%d,", __func__, bdg_rx_v12);
	} else {
		bdg_rx_v12 = RX_V12;
		DDPINFO("%s: not find bdg_rx_v12, set as: %d!", __func__, bdg_rx_v12);
	}

	ret = of_property_read_u32(node, "bdg-rxtx-ratio", &bdg_rxtx_ratio);
	if (!ret) {
		DDPINFO("%s: read bdg_rxtx_ratio:%d,", __func__, bdg_rxtx_ratio);
	} else {
		bdg_rxtx_ratio = 229;
		DDPINFO("%s: not find bdg-rxtx-ratio, set as: %d!", __func__, bdg_rxtx_ratio);
	}

	ret = of_property_read_u32(node, "hs-dco-enable", &hs_dco_enable);
	if (!ret) {
		DDPINFO("%s: read hs_dco_enable:%d,", __func__, hs_dco_enable);
	} else {
		hs_dco_enable = 1;
		DDPINFO("%s: not find hs-dco-enable, set as: %d!", __func__, hs_dco_enable);
	}

	ret = of_property_read_u32(node, "lp-tx-l023-enable", &lp_tx_l023_enable);
	if (!ret) {
		DDPINFO("%s: read lp_tx_l023_enable:%d,", __func__, lp_tx_l023_enable);
	} else {
		lp_tx_l023_enable = 1;
		DDPINFO("%s: not find lp-tx-l023-enable, set as: %d!", __func__, lp_tx_l023_enable);
	}

	ret = of_property_read_u32(node, "g-mode-enable", &g_mode_enable);
	if (!ret) {
		DDPINFO("%s: read g_mode_enable:%d,", __func__, g_mode_enable);
	} else {
		g_mode_enable = 1;
		DDPINFO("%s: not find g-mode-enable, set as: %d!", __func__, g_mode_enable);
	}
}
...
{
	BDG_OUTREG32(NULL, TX_REG[0]->DSI_TX_SELF_PAT_CON0, 0x0FFF0002);
...
void set_ana_mipi_dsi_off(void *cmdq)
{
	...
	BDG_OUTREGBIT(cmdq, struct MIPI_TX_PLL_CON1_REG,
		MIPI_TX_REG->MIPI_TX_PLL_CON1, BDG_RG_DSI_PLL_EN, 0);

	//pll_sdm_iso_en = 1, pll_sdm_pwr_on = 0
	BDG_OUTREGBIT(cmdq, struct MIPI_TX_PLL_PWR_REG,
		MIPI_TX_REG->MIPI_TX_PLL_PWR, BDG_AD_DSI_PLL_SDM_ISO_EN, 1); //[1]
	BDG_OUTREGBIT(cmdq, struct MIPI_TX_PLL_PWR_REG,
		MIPI_TX_REG->MIPI_TX_PLL_PWR, BDG_AD_DSI_PLL_SDM_PWR_ON, 0); //[0]
...
//		}
		...
		BDG_OUTREGBIT(cmdq, struct MIPI_TX_PLL_PWR_REG,
				...
				BDG_AD_DSI_PLL_SDM_PWR_ON, 1);
		udelay(2); /* 1us */
		BDG_OUTREGBIT(cmdq, struct MIPI_TX_PLL_PWR_REG,
				MIPI_TX_REG->MIPI_TX_PLL_PWR,
				BDG_AD_DSI_PLL_SDM_ISO_EN, 0);
		...
		/* SSC config */
		if (dsi->ext->params->bdg_ssc_enable != 0) {
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (data_Rate && (dsi->ext->params->bdg_ssc_enable != 0))
			BDG_OUTREGBIT(cmdq, struct MIPI_TX_PLL_CON2_REG,
					MIPI_TX_REG->MIPI_TX_PLL_CON2,
					BDG_RG_DSI_PLL_SDM_SSC_EN, 1);
		else
			BDG_OUTREGBIT(cmdq, struct MIPI_TX_PLL_CON2_REG,
					MIPI_TX_REG->MIPI_TX_PLL_CON2,
					BDG_RG_DSI_PLL_SDM_SSC_EN, 0);
	}
...
void bdg_dsi_mipi_clk_change(enum DISP_BDG_ENUM module, void *handle, int clk)
{
	...
	if (clk != 0) {
		...
		if ((status & 0x1) == 0x1)
			BDG_OUTREGBIT(handle, struct MIPI_TX_PLL_CON1_REG,
				MIPI_TX_REG->MIPI_TX_PLL_CON1,
				BDG_RG_DSI_PLL_SDM_PCW_CHG, 0);
		else
			BDG_OUTREGBIT(handle, struct MIPI_TX_PLL_CON1_REG,
				MIPI_TX_REG->MIPI_TX_PLL_CON1,
				BDG_RG_DSI_PLL_SDM_PCW_CHG, 1);
	}
}
