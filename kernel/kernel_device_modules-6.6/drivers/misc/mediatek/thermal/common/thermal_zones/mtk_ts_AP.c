...
static __s32 mtkts_AP_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
...
static __s32  mtk_ts_AP_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
	__u64 dwVCriAP2 = 0;
...
static int mtkts_AP_change_mode(struct thermal_zone_device *thermal,
...
static struct thermal_zone_device_ops mtk_AP_dev_ops = {
	...
	.change_mode = mtkts_AP_change_mode,
...
static const struct proc_ops mtkts_AP_fops = {
	.proc_open = mtk_AP_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtk_AP_write,
	.proc_release = single_release,
};

static const struct proc_ops mtkts_AP_param_fops = {
	.proc_open = mtkts_AP_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_AP_param_write,
	.proc_release = single_release,
};
...
static int mtk_AP_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktzBB", trips, num_trip,
						NULL, &mtk_AP_dev_ops, 0, 0, 0,
						interval * 1000);
...
static int mtkts_AP_probe(struct platform_device *pdev)
{
	...
	thermistor_ch0_1 = iio_channel_get(&pdev->dev, "thermistor-ch4");
