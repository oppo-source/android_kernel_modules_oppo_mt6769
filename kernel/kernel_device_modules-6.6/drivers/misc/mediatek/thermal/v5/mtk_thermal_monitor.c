...
(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%19s %d %19s %d %19s %d",
		...
		i++) {
			...
			strscpy(mcdata->conditions[i], conditions[i],
				THERMAL_NAME_LENGTH);
...
int passive_delay_jiffies,int polling_delay_jiffies)
{
	...
	if (IS_ERR(tz)) {
		pr_notice("%s %s fail, err=%ld\n", __func__, type, PTR_ERR(tz));
		return tz;
	}

	if (strcmp(tz->governor->name, mtk_zone_params.governor_name))
		pr_notice("%s %s governor %s, not match %s\n", __func__, type, tz->governor->name,
			mtk_zone_params.governor_name);

	ret = thermal_zone_device_enable(tz);

	if (ret) {
		thermal_zone_device_unregister(tz);
		pr_notice("%s %s thermal_zone_device_enable fail, ret=%d\n", __func__, type, ret);
		return tz;
	}
