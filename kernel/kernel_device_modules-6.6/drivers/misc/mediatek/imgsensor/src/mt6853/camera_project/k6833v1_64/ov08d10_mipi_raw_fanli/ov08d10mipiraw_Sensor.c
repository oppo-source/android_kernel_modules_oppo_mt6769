...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV08D10_SENSOR_ID_FANLI,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= (15.5*0x40))) //base gain = 0x40
	{
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("FANLI_SHINETECH_FRONT_OV08D10MIPI_SetGain = 16");
		}
		else if(iReg>= 0xf8)//gpw
		{
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("FANLI_SHINETECH_FRONT_OV08D10MIPI_SetGain = 160");
		}
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("FANLI_SHINETECH_FRONT_OV08D10MIPI_SetGain = %d",iReg);
		}
	}
	else
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		*sensor_id = return_sensor_id()+2;
		if (*sensor_id == imgsensor_info.sensor_id) {
			if(deviceInfo_register_value == 0x00){
				Eeprom_DataInit(1, OV08D10_SENSOR_ID_FANLI);
				deviceInfo_register_value = 0x01;
			}
			LOG_INF("OV08D10_SENSOR_ID_FANLI get_imgsensor_id success: 0x%x\n", *sensor_id);
			return ERROR_NONE;
		}
		retry--;
		} while (retry > 0);
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		printk("OV08D10_SENSOR_ID_FANLI get_imgsensor_id failed: 0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id()+2;
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("OV08D10_SENSOR_ID_FANLI open success: 0x%x\n", sensor_id);
				break;
			}
			retry--;
		} while(retry > 0);
...
{
	pr_info("[fanli_shinetech_front_ov08d10] preview mode start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[fanli_shinetech_front_ov08d10] capture mode start\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[fanli_shinetech_front_ov08d10] normal_video mode start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[fanli_shinetech_front_ov08d10] hs_video mode start\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[fanli_shinetech_front_ov08d10] slim_video mode start\n");
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[fanli_shinetech_front_ov08d10] custom1 mode start\n");
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable){
		write_cmos_sensor(0xfd, 0x00);
		write_cmos_sensor(0xc2, 0x30);// soft power up
		...
		write_cmos_sensor(0xc2, 0x32);//soft power down
...
UINT32 OV08D10_MIPI_RAW_FANLI_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
