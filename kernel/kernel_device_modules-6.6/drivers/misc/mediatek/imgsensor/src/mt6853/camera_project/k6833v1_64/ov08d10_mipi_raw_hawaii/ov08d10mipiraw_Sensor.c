...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV08D10_SENSOR_ID_HAWAII,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{  3264, 2448,  0, 0, 3264, 2448, 3264, 2448, 0000, 0000, 3264, 2448,  0, 0, 3264, 2448},
	{  3264, 2448,  0, 0, 3264, 2448, 3264, 2448, 0000, 0306, 3264, 1836,  0, 0, 3264, 1836},
	...
	{  3264, 2448,  0, 0, 3264, 2448, 1632, 1224, 0000, 0000, 1632, 1224,  0, 0, 1632, 1224},/*custom1:binning size*/
};
...
static kal_uint32 read_OV08D10_version(void)
{
	  kal_uint32 ov08d10_version_value = 0;
	  ...
	  write_cmos_sensor(0xfd, 0x08);
	  ov08d10_version_value = read_cmos_sensor(0x0e);
	  LOG_INF("read ov08d10 version 0x0e = %d\n", ov08d10_version_value);
	  return ov08d10_version_value;
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= (15.5*0x40))) //base gain = 0x40
	{
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("HAWAII_SHINETECH_FRONT_OV08D10MIPI_SetGain = 16");
		}
		else if(iReg>= 0xf8)//gpw
		{
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("HAWAII_SHINETECH_FRONT_OV08D10MIPI_SetGain = 160");
		}
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("HAWAII_SHINETECH_FRONT_OV08D10MIPI_SetGain = %d",iReg);
		}
	}
	else
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		*sensor_id = return_sensor_id()+1;
		if (*sensor_id == imgsensor_info.sensor_id) {
			if ((read_OV08D10_version()) < OV08D10_ECO_VALUE) {
				*sensor_id = 0xFFFFFFFF;
				return ERROR_SENSOR_CONNECT_FAIL;
			} else {
                	if(deviceInfo_register_value == 0x00){
                    	Eeprom_DataInit(1, OV08D10_SENSOR_ID_HAWAII);
                        deviceInfo_register_value = 0x01;
                	}
				LOG_INF("OV08D10_SENSOR_ID_HAWAII get_imgsensor_id success: 0x%x\n", *sensor_id);
				return ERROR_NONE;
			}
		}
		retry--;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		printk("OV08D10_SENSOR_ID_HAWAII get_imgsensor_id failed: 0x%x\n", *sensor_id);
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id()+1;
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("OV08D10_SENSOR_ID_HAWAII open success: 0x%x\n", sensor_id);
				break;
			}
			retry--;
		} while(retry > 0);
...
{
	pr_info("[hawaiis_shinetech_front_ov08d10] preview mode start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[hawaii_shinetech_front_ov08d10] capture mode start\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[hawaii_shinetech_front_ov08d10] normal_video mode start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[hawaii_shinetech_front_ov08d10] hs_video mode start\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[hawaii_shinetech_front_ov08d10] slim_video mode start\n");
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[hawaii_shinetech_front_ov08d10] custom1 mode start\n");
		...
		UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	/*case SENSOR_FEATURE_GET_SERIANO_IC:
		read_hawaii_shinetech_front_ov08d10_QRSerialdata(feature_data_8, feature_para_len);
...
UINT32 OV08D10_MIPI_RAW_HAWAII_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
