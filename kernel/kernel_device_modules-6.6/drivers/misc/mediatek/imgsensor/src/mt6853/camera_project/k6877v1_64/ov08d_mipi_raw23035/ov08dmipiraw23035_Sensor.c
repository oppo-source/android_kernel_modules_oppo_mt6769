...
#include "ov08dmipiraw23035_Sensor.h"
#define PFX "Avatar_ov08d_mipiraw_sensor"
#define LOG_INF(format, args...)    \
	pr_debug(PFX "[%s] " format, __func__, ##args)
#define MULTI_WRITE 0
#define OV08D_SENSOR_ID 0x56084700
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV08D_SENSOR_ID23035,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7] = {
	...
	{  3264, 2448,  0, 0, 3264, 2448, 3264, 2448, 0000,  296, 3264, 1856,  0, 0, 3264, 1856},
};
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == OV08D_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				pr_info("Avatar Ov08d Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				if (deviceInfo_register_value == 0x00) {
					Eeprom_DataInit(1, OV08D_SENSOR_ID23035);
					deviceInfo_register_value=0x01;
				}
				if (otp_data[0] == 0) {
					read_otp_info(otp_data);
				} else {
					pr_err("Avatar otp data has already read");
				}
				LOG_INF("RENM0_module_id=%d\n",imgsensor_info.module_id);
				return ERROR_NONE;
			}
			pr_info("Avatar Ov08d Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("normal_video mode start\n");
		...
		UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MINT32 *)(signed long)(*(feature_data + 1)) = -1911000;
		break;
...
UINT32 OV08D_MIPI_RAW23035_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
