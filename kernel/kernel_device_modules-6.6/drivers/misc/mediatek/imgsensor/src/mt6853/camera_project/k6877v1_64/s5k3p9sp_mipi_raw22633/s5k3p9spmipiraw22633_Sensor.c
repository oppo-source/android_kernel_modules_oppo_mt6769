...
#include "s5k3p9spmipiraw22633_Sensor.h"
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] =
{
	...
	{ 4640, 3488,	  8,	448, 4624, 2592, 2312,  1296, 0004, 0000, 2304,  1296,	  0,	0, 2304, 1296}, /*video*/
...
static void burst_read_to_check(void)
{
	...
	{
		iReadRegI2CTiming(addr_0x6F12, 2, (u8*)&get_byte, 2, imgsensor.i2c_write_id, imgsensor_info.i2c_speed);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	*/
	...
	printk("3p9 get_imgsensor_id----1  0x%x\n",imgsensor_info.sensor_id);
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001));
			printk("3p9 get_imgsensor_id----2  0x%x\n",*sensor_id);
			...
			if (*sensor_id == S5K3P9SP_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				...
				if (deviceInfo_register_value == 0x00) {
					Eeprom_DataInit(1, S5K3P9SP_SENSOR_ID22633);
					deviceInfo_register_value = 0x01;
				}
				#endif
...
UINT32 S5K3P9SP_MIPI_RAW22633_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
