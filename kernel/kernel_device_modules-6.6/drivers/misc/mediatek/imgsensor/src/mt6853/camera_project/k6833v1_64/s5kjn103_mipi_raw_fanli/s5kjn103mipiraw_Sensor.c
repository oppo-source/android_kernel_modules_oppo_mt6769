...
#define PFX "fanli s5kjn103_camera_sensor"
#define LOG_1 LOG_INF("fanli s5kjn103,MIPI 4LANE\n")
/****************************   Modify end	**************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN103_SENSOR_ID_FANLI,
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == S5KJN103_SENSOR_ID_FANLI) && (data_length == DUALCAM_CALI_DATA_LENGTH)
			...
			if (*sensor_id == S5KJN103_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				...
				if(deviceInfo_register_value == 0x00){
					ret = Eeprom_DataInit(0, S5KJN103_SENSOR_ID_FANLI);
...
UINT32 S5KJN103_MIPI_RAW_FANLI_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
