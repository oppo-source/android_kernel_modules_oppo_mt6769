...
#include "aladdin_ov08d10mipiraw_Sensor.h"
#define EEPROM_WRITE_ID 0xA0
#define OV08D10_ECO_VALUE 3
#define PFX "ALADDIN_FRONT_OV08D10_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = ALADDIN_OV08D10_SENSOR_ID,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= (15.5*0x40))) //base gain = 0x40
	{
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("ALADDIN_OV08D10MIPI_SetGain = 16");
		}
		else if(iReg>= 0xf8)//gpw
		{
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("ALADDIN_OV08D10MIPI_SetGain = 160");
		}
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("ALADDIN_OV08D10MIPI_SetGain = %d",iReg);
		}
	}
	else
...
static kal_uint16 aladdin_shinetech_ov08d10_init_setting[] = {
	0xfd, 0x00,
};

static kal_uint16 aladdin_shinetech_ov08d10_preview_setting[] = {
	0xfd, 0x00,
	0x20, 0x0e,
	0x20, 0x0b,
	0xfd, 0x00,
...
static kal_uint16 aladdin_shinetech_ov08d10_capture_setting[] = {
	0xfd, 0x00,
	0x20, 0x0e,
	0x20, 0x0b,
	0xfd, 0x00,
...
static kal_uint16 aladdin_shinetech_ov08d10_normal_video_setting[] = {
	0xfd, 0x00,
	0x20, 0x0e,
	0x20, 0x0b,
	0xfd, 0x00,
...
{
	LOG_INF("start");
	table_write_cmos_sensor(aladdin_shinetech_ov08d10_init_setting,
		sizeof(aladdin_shinetech_ov08d10_init_setting) / sizeof(kal_uint16));
	LOG_INF("end");
}
...
{
	LOG_INF("start");
	table_write_cmos_sensor(aladdin_shinetech_ov08d10_preview_setting,
		sizeof(aladdin_shinetech_ov08d10_preview_setting) / sizeof(kal_uint16));
	LOG_INF("end");
}
...
{
	LOG_INF("start");
	table_write_cmos_sensor(aladdin_shinetech_ov08d10_capture_setting,
		sizeof(aladdin_shinetech_ov08d10_capture_setting) / sizeof(kal_uint16));
	LOG_INF("end");
}
...
{
	LOG_INF("start");
	table_write_cmos_sensor(aladdin_shinetech_ov08d10_normal_video_setting,
		sizeof(aladdin_shinetech_ov08d10_normal_video_setting) / sizeof(kal_uint16));
	LOG_INF("end");
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		printk("ALADDIN_OV08D10 get_imgsensor_id failed: 0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id() + 11;
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("ALADDIN_OV08D10 open success: 0x%x\n", sensor_id);
				break;
			}
			retry--;
		} while(retry > 0);
...
{
	pr_info("[aladdin_ov08d10] preview mode start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[aladdin_ov08d10] capture mode start\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[aladdin_ov08d10] normal_video mode start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[aladdin_ov08d10] hs_video mode start\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[aladdin_ov08d10] slim_video mode start\n");
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[aladdin_ov08d10] custom1 mode start\n");
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if(enable) {
		...
		LOG_INF("ov08d10 enter pattern_mode not enable");
		write_cmos_sensor(0xfd,0x01);
		...
		UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	/*case SENSOR_FEATURE_GET_SERIANO_IC:
		read_aladdin_ov08d10_QRSerialdata(feature_data_8, feature_para_len);
