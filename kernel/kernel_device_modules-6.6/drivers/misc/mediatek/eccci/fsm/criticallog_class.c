...
EXPORT_SYMBOL(criticallog_set_state);

static int create_criticallog_class(void)
{
	if (!criticallog_class) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
		criticallog_class = class_create("critical_log");
#else
...
int criticallog_dev_register(struct criticallog_dev *sdev)
{
	...
	MKDEV(0, sdev->index), NULL, "%s", g_criticallog_name);
	if (IS_ERR(sdev->dev))
...
EXPORT_SYMBOL(criticallog_dev_register);
...
EXPORT_SYMBOL(criticallog_dev_unregister);

int criticallog_class_init(void)
{
	return create_criticallog_class();
}
EXPORT_SYMBOL(criticallog_class_init);
