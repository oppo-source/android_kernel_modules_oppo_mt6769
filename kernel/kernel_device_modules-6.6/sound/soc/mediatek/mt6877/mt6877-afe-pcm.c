...
static int mt6877_fe_startup(struct snd_pcm_substream *substream,
...
void mt6877_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6877_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int mt6877_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int mt6877_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6877_rate_transform(afe->dev, rate, dai_id);
}
...
int mt6877_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6877_MEMIF_PBUF_SIZE_256_BYTES;
	else
...
/* FE DAIs */
static const struct snd_soc_dai_ops mt6877_memif_dai_ops = {
	.startup	= mt6877_fe_startup,
...
static struct snd_soc_dai_driver mt6877_memif_dai_driver[] = {
	/* FE DAIs: memory intefaces to CPU */
	{
		...
		.ops = &mt6877_memif_dai_ops,
	},
	{
		.name = "DL12",
		.id = MT6877_MEMIF_DL12,
		.playback = {
			.stream_name = "DL12",
			.channels_min = 1,
			.channels_max = 2,
...
/* kcontrol */
static int mt6877_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6877_PRIMARY_MEMIF];
	return 0;
}

static int mt6877_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6877_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6877_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6877_RECORD_MEMIF];
	return 0;
}

static int mt6877_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6877_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6877_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6877_DEEP_MEMIF];
	return 0;
}

static int mt6877_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6877_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6877_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6877_VOIP_MEMIF];
	return 0;
}

static int mt6877_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6877_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6877_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6877_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6877_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6877_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6877_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6877_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6877_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6877_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6877_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6877_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

static int mt6877_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6877_BARGEIN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6877_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6877_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6877_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6877_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6877_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6877_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6877_dl_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6877_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6877_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static const struct snd_kcontrol_new mt6877_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6877_adsp_ref_mem_get,
		       mt6877_adsp_ref_mem_set),
	...
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6877_mmap_ul_scene_get, mt6877_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6877_dl_mmap_fd_get,
		       mt6877_dl_mmap_fd_set),
...
static const struct snd_soc_dapm_widget mt6877_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6877_memif_routes[] = {
	{"UL1", NULL, "UL1_CH1"},
	{"UL1", NULL, "UL1_CH2"},
	{"UL1", NULL, "UL1_CH3"},
	{"UL1", NULL, "UL1_CH4"},
...
static const struct mtk_base_irq_data irq_data[MT6877_IRQ_NUM] = {
	[MT6877_IRQ_0] = {
		.id = MT6877_IRQ_0,
...
static const int memif_irq_usage[MT6877_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6877_MEMIF_DL1] = MT6877_IRQ_0,
	[MT6877_MEMIF_DL2] = MT6877_IRQ_1,
	[MT6877_MEMIF_DL3] = MT6877_IRQ_2,
	[MT6877_MEMIF_DL4] = MT6877_IRQ_3,
	[MT6877_MEMIF_DL5] = MT6877_IRQ_4,
	[MT6877_MEMIF_DL6] = MT6877_IRQ_5,
	[MT6877_MEMIF_DL7] = MT6877_IRQ_6,
	[MT6877_MEMIF_DL8] = MT6877_IRQ_7,
	[MT6877_MEMIF_DL9] = MT6877_IRQ_8,
	[MT6877_MEMIF_DL12] = MT6877_IRQ_9,
	[MT6877_MEMIF_DAI] = MT6877_IRQ_10,
	[MT6877_MEMIF_MOD_DAI] = MT6877_IRQ_11,
	[MT6877_MEMIF_DAI2] = MT6877_IRQ_12,
	[MT6877_MEMIF_VUL12] = MT6877_IRQ_13,
	[MT6877_MEMIF_VUL2] = MT6877_IRQ_14,
	[MT6877_MEMIF_AWB] = MT6877_IRQ_15,
	[MT6877_MEMIF_AWB2] = MT6877_IRQ_16,
	[MT6877_MEMIF_VUL3] = MT6877_IRQ_17,
	[MT6877_MEMIF_VUL4] = MT6877_IRQ_18,
	[MT6877_MEMIF_VUL5] = MT6877_IRQ_19,
	[MT6877_MEMIF_VUL6] = MT6877_IRQ_20,
};

static bool mt6877_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6877_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6877_is_volatile_reg,
...
static int mt6877_afe_runtime_suspend(struct device *dev)
{
...
static int mt6877_set_memif_sram_mode(struct device *dev,
...
static int mt6877_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	/* set memif sram mode */
	mt6877_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6877_sram_ops = {
...
static const struct mtk_afe_debug_cmd mt6877_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6877_debugfs_ops = {
...
static int mt6877_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6877_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6877_memif_dai_driver);

	dai->controls = mt6877_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6877_pcm_kcontrols);
	dai->dapm_widgets = mt6877_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6877_memif_widgets);
	dai->dapm_routes = mt6877_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6877_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6877_dai_adda_register,
	mt6877_dai_i2s_register,
	mt6877_dai_hw_gain_register,
	mt6877_dai_src_register,
	mt6877_dai_pcm_register,
	mt6877_dai_hostless_register,
	mt6877_dai_memif_register,
};
...
static int mt6877_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	afe->memif[MT6877_DEEP_MEMIF].ack = mtk_sp_clean_written_buffer_ack;
	...
	/* others */
	afe->mtk_afe_hardware = &mt6877_afe_hardware;
	afe->memif_fs = mt6877_memif_fs;
	afe->irq_fs = mt6877_irq_fs;
	afe->get_dai_fs = mt6877_get_dai_fs;
	afe->get_memif_pbuf_size = mt6877_get_memif_pbuf_size;

	afe->runtime_resume = mt6877_afe_runtime_resume;
	afe->runtime_suspend = mt6877_afe_runtime_suspend;

	afe->request_dram_resource = mt6877_afe_dram_request;
	afe->release_dram_resource = mt6877_afe_dram_release;
...
static int mt6877_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	pm_runtime_disable(&pdev->dev);
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6877_afe_runtime_suspend(&pdev->dev);
...
static const struct of_device_id mt6877_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt6877-sound", },
	{},
};
MODULE_DEVICE_TABLE(of, mt6877_afe_pcm_dt_match);

static const struct dev_pm_ops mt6877_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6877_afe_runtime_suspend,
			   mt6877_afe_runtime_resume, NULL)
};

static struct platform_driver mt6877_afe_pcm_driver = {
	.driver = {
		   .name = "mt6877-audio",
	...
	.probe = mt6877_afe_pcm_dev_probe,
	.remove = mt6877_afe_pcm_dev_remove,
};
