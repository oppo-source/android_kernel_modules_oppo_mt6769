    ...
    native.filegroup(
        ...
        srcs = native.glob(
            ...
            exclude = [
                ...
                "**/*.bzl",
                "build.config.*",
                "drivers/power/oplus/.git/**",
                "drivers/misc/oplus/vibrator/.git/**",
            ],
        ),
    )
    ...
    for build in ["eng", "userdebug", "user", "ack"]:
        ...
        if True:
            ...
            mgk_build_config(
                ...
                oplus_compile_platform = oplus_compile_platform,
            )
        if build != "ack":
...
""")
    if ctx.attr.gki_mixed_build or (ctx.attr.build_variant != "ack"):
        ...
        if ctx.attr.oplus_compile_platform == "k6991v1_64":
            defconfig.append("${ROOT_DIR}/" + ctx.attr.device_modules_dir + "/kernel/configs/oplus6991.config")
        if ctx.attr.oplus_compile_platform == "k6877v1_64":
            defconfig.append("${ROOT_DIR}/" + ctx.attr.device_modules_dir + "/kernel/configs/oplus6877.config")
        if ctx.attr.oplus_compile_platform == "k69v1_64":
            defconfig.append("${ROOT_DIR}/" + ctx.attr.device_modules_dir + "/kernel/configs/oplus6769.config")
        if ctx.attr.oplus_compile_platform == "k6833v1_64":
            defconfig.append("${ROOT_DIR}/" + ctx.attr.device_modules_dir + "/kernel/configs/oplus6833.config")
        if is_kasan_load == 1:
            defconfig = defconfig + kasan_config
        content.append("DEFCONFIG={}".format(ctx.attr.defconfig))
        print("_mgk_build_config_impl defconfig is %s" %(defconfig))
...
mgk_build_config = rule(
    ...
    attrs = {
        ...
        "oplus_compile_platform": attr.string(),
    },
)
