...
#if IS_ENABLED(CONFIG_MALI_MTK_POWER_TRANSITION_TIMEOUT_DEBUG)
	ret_warn = WARN_ON(scheduler->state != SCHED_SLEEPING);
	ret_warn |= WARN_ON(!kbdev->pm.backend.gpu_sleep_mode_active);
	ret_warn |= WARN_ON(kbdev->pm.active_count);
	ret_warn |= WARN_ON(kbdev->pm.backend.gpu_sleep_mode_active && kbdev->pm.backend.exit_gpu_sleep_mode);
	if (ret_warn)
...
static int term_group_sync(struct kbase_queue_group *group)
{
	...
	const unsigned int group_term_timeout_ms = kbase_get_timeout_ms(kbdev, kbdev->csf.csg_term_timeout_ms);
#else
	...
	if (unlikely(!remaining)) {
		...
		mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_CRITICAL | MTK_LOGBUFFER_TYPE_EXCEPTION,
			...
			group_term_timeout_ms, group->handle,
			group->kctx->tgid, group->kctx->id, group->csg_nr);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_CRITICAL | MTK_LOGBUFFER_TYPE_EXCEPTION,
				"Timeout (%d ms) waiting for CSG slots to start, slots: 0x%*pb\n",
				kbase_get_timeout_ms(kbdev, CSF_FIRMWARE_TIMEOUT),
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
	mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_CRITICAL | MTK_LOGBUFFER_TYPE_EXCEPTION,
		 "Evicting context %d_%d slots: 0x%*pb\n", kctx->tgid, kctx->id,
		 num_groups, slot_mask);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER) && IS_ENABLED(CONFIG_MALI_MTK_DEBUG_DUMP)
			dev_warn(
				kbdev->dev,
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_CRITICAL | MTK_LOGBUFFER_TYPE_EXCEPTION,
			"Timeout waiting for CACHE_CLN_INV_L2_LSC\n");
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
		if (!ret)
			ret = ret2;
	}
