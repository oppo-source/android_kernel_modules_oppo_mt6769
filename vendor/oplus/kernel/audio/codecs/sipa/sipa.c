...
int sipa_get_pa_status(int channel);
#ifdef OPLUS_FEATURE_SPEAKER_MUTE
void sipa_speaker_mute_set(struct oplus_speaker_device *speaker_device, int enable);
	...
	unsigned int cycles)
{
	...
	/* wait for owi reset, longer than 5ms */
...
int sipa_pa_enable_by_scene(int channel, unsigned int scene, int enable)
{
	sipa_dev_t *si_pa = NULL;
	int i = 0;
...
int sipa_get_speaker_status(struct oplus_speaker_device *speaker_device)
{
	int status = 0;
	int channel = 0;
	...
	status = sipa_get_pa_status(channel);
...
int sipa_speaker_enable(struct oplus_speaker_device *speaker_device, int enable)
{
	int channel = 0;
	...
	sipa_pa_enable_by_scene(channel, scene, enable);
...
int sipa_volme_boost_set(struct oplus_speaker_device *speaker_device, int level)
{
	...
	if (si_pa != NULL) {
		if (channel == 0) {
			if((0 != sipa_regmap_write(si_pa->regmap, si_pa->chip_type, addr, 1, &val_L))) {
				pr_err("%s: regmap_write_L \n", __func__);
			}
		} else if (channel == 1) {
			if((0 != sipa_regmap_write(si_pa->regmap, si_pa->chip_type, addr, 1, &val_R))) {
...
static int sipa_scene_set(struct sipa_dev_s *si_pa, unsigned int scene)
{
	...
	if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
		// do not operate i2c while the i2s signal stopped
	...
	char *buf)
{
	...
	tb = kmalloc(BUF_SIZE, GFP_KERNEL);
	if (!tb) {
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: freq support = %ld, channle = %d \n",
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: min freq = %ld, channle = %d \n",
			__func__, ucontrol->value.integer.value[0], si_pa->channel_num);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: max freq = %ld, channle = %d \n",
			__func__, ucontrol->value.integer.value[0], si_pa->channel_num);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: min mohms = %ld, channle = %d \n",
			__func__, ucontrol->value.integer.value[0], si_pa->channel_num);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: max mohms = %ld, channle = %d \n",
			__func__, ucontrol->value.integer.value[0], si_pa->channel_num);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: default mohms = %ld, channle = %d \n",
			__func__, ucontrol->value.integer.value[0], si_pa->channel_num);
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
static int detect_i2c_slave(sipa_dev_t *si_pa)
{
	if (0 == si_pa->disable_pin && !IS_SUPPORT_OWI_TYPE(si_pa->chip_type)) {
...
int sipa_i2c_probe(struct i2c_client *client)
#else
int sipa_i2c_probe(
...
#endif
{
	...
	if ((!IS_DIGITAL_PA_TYPE(si_pa->chip_type)) && (detect_i2c_slave(si_pa) == 0)) {
...
out1:
	...
	if (ret != 0) {
		mm_fb_audio_kevent_named_delay(OPLUS_AUDIO_EVENTID_SMARTPA_ERR, MM_FB_KEY_RATELIMIT_5MIN, \
			FEEDBACK_DELAY_60S, "payload@@%s: out1 error %d", LOG_FLAG, ret);
	}
#endif
...
out0:
	...
	if (ret != 0) {
		mm_fb_audio_kevent_named_delay(OPLUS_AUDIO_EVENTID_SMARTPA_ERR, MM_FB_KEY_RATELIMIT_5MIN, \
			FEEDBACK_DELAY_60S, "payload@@%s: out0 error %d", LOG_FLAG, ret);
	}
#endif
	put_sipa_dev(si_pa);
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
