...
retry:
	for_each_cpu(cpu, &ux_cputopo.sched_cls[cls_nr].cpus) {
		...
		if (oplus_get_ux_state(rq->curr) & (SA_TYPE_HEAVY | SA_TYPE_LIGHT | SA_TYPE_ANIMATOR | SA_TYPE_LISTPICK))
			continue;
...
void inc_ld_stats(struct task_struct *tsk, struct rq *rq)
{
	...
	int curr_high_load = ots->lb_state & 0x1;
	int curr_task_type = (ots->lb_state >> 1) & 0x7;
