...
int ili_ic_whole_reset(bool mcu)
{
	...
	ret = ili_ice_mode_write(ilits->chip->reset_addr, ilits->chip->reset_key,
				 sizeof(u32));
...
int ili_ic_int_trigger_ctrl(bool pulse)
{
	...
	if (ili_ic_func_ctrl("int_trigger", pulse) < 0) {
		ILI_ERR("Write CMD error, set back to <%s> trigger\n",
			ilits->pulse_state ? "Level" : "Pulse");
		return -1;
	}

	ilits->pulse_state = pulse;
...
{
	if (!wait_event_interruptible_timeout(ilits->inq,
					      !atomic_read(&ilits->cmd_int_check),
...
int ili_ic_get_core_ver(void)
{
	...
	do {
		...
		if (ilits->wrapper(&cmd[1], sizeof(u8), buf, ilits->protocol->core_ver_len, ON,
				   OFF) < 0) {
...
int ili_ic_get_fw_ver(void)
{
	...
	if (ilits->wrapper(&cmd[1], sizeof(u8), buf, ilits->protocol->fw_ver_len, ON,
			   OFF) < 0) {
...
out:
	...
	if (ilits->ts->panel_data.manufacture_info.version) {
		...
		if (ilits->ts->panel_data.vid_len == 0) {
			...
			if (ver_len <= 11) {
				snprintf(ilits->ts->panel_data.manufacture_info.version + 9,
...
{
	if ((ilits->ts->resolution_info.max_x != 0
			&& ilits->ts->resolution_info.max_y != 0)) {
...
int ili_ic_get_tp_info(void)
{
	...
	ret = ilits->wrapper(&cmd[1], sizeof(u8), buf, ilits->protocol->tp_info_len, ON,
			     OFF);
...
int ili_ic_get_protocl_ver(void)
{
	...
	if (ilits->wrapper(&cmd[1], sizeof(u8), buf, ilits->protocol->pro_ver_len, ON,
			   OFF) < 0) {
