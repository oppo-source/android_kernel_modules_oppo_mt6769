...
#include "nvt_drivers_nt36528_noflash.h"
...
static const struct nvt_ts_mem_map NT36528A_memory_map = {
	...
	.DOZE_GM_S1D_SCAN_RAW_ADDR = 0x21F00,
...
static const struct nvt_ts_trim_id_table trim_id_table[] = {
	{
		.id = {0x0A, 0xFF, 0xFF, 0x28, 0x65, 0x03},
		.mask = {1, 0, 0, 1, 1, 1},
		.mmap = &NT36528A_memory_map,
		.hwinfo = &NT36528A_hw_info
	},
	{
		.id = {0x0B, 0xFF, 0xFF, 0x28, 0x65, 0x03},
		.mask = {1, 0, 0, 1, 1, 1},
		.mmap = &NT36528A_memory_map,
		.hwinfo = &NT36528A_hw_info
	},
...
static void nvt_sw_reset_idle(struct chip_data_nt36528 *chip_info)
{
...
return:
	n.a.
*******************************************************/
static void nvt_bootloader_reset_noflash(struct chip_data_nt36528 *chip_info)
{
...
return:
	Executive outcomes. 0---succeed. -5---access fail.
*******************************************************/
static int32_t nvt_set_page(struct chip_data_nt36528 *chip_info, uint32_t addr)
{
	uint8_t buf[4] = {0};
...
return:
	Executive outcomes. 0---succeed. -5---access fail.
*******************************************************/
int32_t nvt_read_reg(struct chip_data_nt36528 *chip_info, nvt_ts_reg_t reg, uint8_t *val)
{
	int32_t ret = 0;
	uint32_t addr = 0;
	uint8_t mask = 0;
...
static uint8_t nvt_wdt_fw_recovery(struct chip_data_nt36528 *chip_info,
				   uint8_t *point_data)
{
...
static int nvt_output_data(int *buffer, struct chip_data_nt36528 *ts_data,
...
static void nvt_esd_check_update_timer(struct chip_data_nt36528 *chip_info)
{
	TPD_DEBUG("%s\n", __func__);
...
static void nvt_esd_check_enable(struct chip_data_nt36528 *chip_info, bool enable)
{
...
*******************************************************/
static int8_t nvt_ts_check_chip_ver_trim(struct chip_data_nt36528 *chip_info,
		struct nvt_ts_hw_reg_addr_info hw_regs)
{
	...
	if (chip_info->trim_id_table.mmap == NULL) {  /*set default value */
		chip_info->trim_id_table.mmap = &NT36528A_memory_map;
		chip_info->hw_crc = HWCRC_LEN_3Bytes;
		chip_info->auto_copy = AUTOCOPY_NOSUPPORT;
	}
...
return:
	Executive outcomes. 0---NVT IC. -1---not NVT IC.
*******************************************************/
static int32_t nvt_ts_check_chip_ver_trim_loop(struct chip_data_nt36528 *chip_info) {
	uint8_t i = 0;
	int32_t ret = 0;
...
static int32_t nvt_check_fw_reset_state_noflash(
	struct chip_data_nt36528 *chip_info, RST_COMPLETE_STATE check_reset_state)
{
	uint8_t buf[8] = {0};
	int32_t ret = 0;
	int32_t retry = 0;
...
static int nvt_enter_sleep(struct chip_data_nt36528 *chip_info, bool config)
{
	int ret = -1;
...
static int32_t nvt_get_fw_info_noflash(struct chip_data_nt36528 *chip_info)
{
...
static int32_t nvt_bin_header_parser(struct chip_data_nt36528 *chip_info,
...
return:
	n.a.
*******************************************************/
static int32_t Download_Init(struct chip_data_nt36528 *chip_info)
{
...
return:
	n.a.
*******************************************************/
static int32_t Write_Partition(struct chip_data_nt36528 *chip_info,
				   const u8 *fwdata, size_t fwsize)
{
	uint32_t list = 0;
	char *name;
...
return:
	N/A.
*******************************************************/
static void nvt_fw_crc_enable(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[4] = {0};
...
return:
	Executive outcomes. 0---succeed. -5---access fail.
*******************************************************/
static void nvt_boot_ready(struct chip_data_nt36528 *chip_info, uint8_t ready)
{
...
return:
	N/A.
*******************************************************/
void nvt_tx_auto_copy_mode(struct chip_data_nt36528 *chip_info)
{
...
return:
	N/A.
*******************************************************/
int32_t nvt_check_spi_dma_tx_info(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
...
*******************************************************/
int32_t nvt_check_tx_auto_copy(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
	const int32_t retry = 200;
...
return:
	Executive outcomes. 0---succeed. -1---fail.
*******************************************************/
int32_t nvt_wait_auto_copy(struct chip_data_nt36528 *chip_info)
{
	if (chip_info->auto_copy == CHECK_SPI_DMA_TX_INFO) {
...
return:
	n.a.
*******************************************************/
static void nvt_set_bld_crc_bank(struct chip_data_nt36528 *chip_info,
...
return:
	n.a.
*******************************************************/
static void nvt_set_bld_hw_crc(struct chip_data_nt36528 *chip_info)
{
	/* [0] ILM */
	/* write register bank */
...
return:
	n.a.
*******************************************************/
static void nvt_read_bld_hw_crc(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[8] = {0};
...
return:
	n.a.
*******************************************************/
static int32_t Download_Firmware_HW_CRC(struct chip_data_nt36528 *chip_info,
...
static int32_t nvt_nf_detect_chip(struct chip_data_nt36528 *chip_info)
{
	int32_t ret = 0;
	int i;
	uint8_t buf[8] = {0};
...
static void nova_write_log_buf(struct chip_data_nt36528 *chip_info, u8 main_id,
				   u8 sec_id)
{
...
static void nova_debug_info(struct chip_data_nt36528 *chip_info, u8 *buf)
{
...
/*			switch (chip_info->ts->orientation) {
	...
	if (chip_info->debug_mode_sta) {
		nova_debug_info(chip_info, &point_data[111]);
	}
...
static void nvt_read_debug_gesture_coordinate_buffer(
							struct chip_data_nt36528 *chip_info,
...
static void nvt_dbg_gesture_record_coor_read(
				struct chip_data_nt36528 *chip_info, u8 pointdata)
{
	u8 *xdata = NULL;
	uint32_t buf_len = 0;
...
#ifdef CONFIG_OPLUS_TP_APK
static __maybe_unused int nvt_enable_debug_gesture_coordinate_record_mode(struct chip_data_nt36528 *chip_info, bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int nvt_enable_debug_gesture_coordinate_mode(struct chip_data_nt36528 *chip_info, bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_black_gesture(struct chip_data_nt36528 *chip_info, bool enable)
{
	int ret = -1;
...
static int nvt_enable_edge_limit(struct chip_data_nt36528 *chip_info,
				int state)
{
	int8_t ret = -1;
...
/*	case LANDSCAPE_SCREEN_180:
		ret = nvt_extend_cmd_store(chip_info,
			EVENTBUFFER_EDGE_LIMIT_VERTICAL_REVERSE, NUM_OF_MONITOR_FRAMES);
		break; */
...
static int nvt_enable_charge_mode(struct chip_data_nt36528 *chip_info,
				bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_game_mode(struct chip_data_nt36528 *chip_info,
				bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_headset_mode(struct chip_data_nt36528 *chip_info,
				bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int nvt_enable_hopping_fix_freq_mode(struct chip_data_nt36528 *chip_info, bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int nvt_enable_debug_msg_diff_mode(struct chip_data_nt36528 *chip_info, bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int nvt_enable_water_polling_mode(struct chip_data_nt36528 *chip_info, bool enable)
{
	int8_t ret = -1;
...
static int32_t nvt_clear_fw_status(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
...
static void nvt_enable_noise_collect(struct chip_data_nt36528 *chip_info,
					 int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = {0};
...
static int8_t nvt_switch_FreqHopEnDis(struct chip_data_nt36528 *chip_info,
...
static void nvt_change_mode(struct chip_data_nt36528 *chip_info, uint8_t mode)
{
	uint8_t buf[8] = {0};
...
static uint8_t nvt_get_fw_pipe_noflash(struct chip_data_nt36528 *chip_info)
{
	int ret = -1;
	uint8_t buf[8] = {0};
...
static void nvt_read_mdata(struct chip_data_nt36528 *chip_info,
...
#ifdef CONFIG_OPLUS_TP_APK
static void nvt_read_debug_mdata(struct chip_data_nt36528 *chip_info,
				 uint32_t xdata_addr, int32_t *xdata, int32_t xdata_len)
{
	int32_t i = 0;
	int32_t j = 0;
...
static int32_t nvt_check_fw_status(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
...
static uint8_t nvt_get_fw_pipe(struct chip_data_nt36528 *chip_info)
{
	int ret = -1;
	uint8_t buf[8] = {0};
...
static int32_t nvt_read_fw_noise(struct chip_data_nt36528 *chip_info,
...
static int32_t nvt_enter_digital_test(struct chip_data_nt36528 *chip_info,
		...
		const struct firmware *fw, bool force)
{
	...
	struct chip_data_nt36528 *chip_info = NULL;
	...
	chip_info = (struct chip_data_nt36528 *)chip_data;
...
static void nvt_enable_doze_noise_collect(struct chip_data_nt36528 *chip_info, int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = {0};
...
static int32_t nvt_read_doze_fw_noise(struct chip_data_nt36528 *chip_info,
...
static int32_t nvt_read_doze_baseline(struct chip_data_nt36528 *chip_info,
					  int32_t doze_x_channel, int32_t *xdata, int32_t xdata_len)
{
	...
	for (x = 0; x < doze_x_channel; x++) {
		...
		nvt_set_page(chip_info, chip_info->trim_id_table.mmap->DOZE_GM_S1D_SCAN_RAW_ADDR
				+ rx_num * doze_x_channel * x);

		/*---read data---*/
		buf[0] = (chip_info->trim_id_table.mmap->DOZE_GM_S1D_SCAN_RAW_ADDR + rx_num *
			doze_x_channel * x) & 0xFF;
		CTP_SPI_READ(chip_info->s_client, buf, rx_num * doze_x_channel + 1);
		...
		struct test_item_info *p_test_item_info)
{
	...
	TPD_INFO("%s : update test firmware successded\n", __func__);
	...
	chip_info->p_nvt_test_para->doze_x_channel					= 2; /* chip_info->hw_res->doze_x_num */
	...
	chip_info->p_nvt_test_para->config_lmt_lpwg_diff_n			= 0;
	chip_info->p_nvt_test_para->fdm_x_channel					 = 2; /* chip_info->hw_res->doze_x_num */
...
static void nvt_data_read(struct seq_file *s,
			struct chip_data_nt36528 *chip_info, DEBUG_READ_TYPE read_type)
{
	int ret = -1;
	int i, j;
	uint8_t pipe;
...
int32_t nvt_set_pen_inband_mode_1(struct chip_data_nt36528 *chip_info, uint8_t freq_idx, uint8_t x_term)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
...
int32_t nvt_set_pen_normal_mode(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
	const int32_t retry = 5;
...
static void nvt_enable_short_test(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[8] = {0};
...
static int32_t nvt_read_fw_short(struct chip_data_nt36528 *chip_info,
...
static void nvt_enable_open_test(struct chip_data_nt36528 *chip_info)
{
	uint8_t buf[8] = {0};
...
static int32_t nvt_read_fw_open(struct chip_data_nt36528 *chip_info,
				int32_t *xdata, int32_t xdata_len)
{
	uint32_t raw_pipe_addr = 0;
	uint8_t *rawdata_buf = NULL;
				  ...
				  struct auto_testdata *nvt_testdata, struct test_item_info *p_test_item_info)
{
	...
	if (chip_info->p_nvt_autotest_offset->digital_diff_p
		...
		if (nvt_enter_digital_test(chip_info, true)) {
			TPD_INFO("enter digital test failed!\n");
			digital_noise_p_result = -NVT_MP_FAIL_READ_DATA;
			digital_noise_n_result = -NVT_MP_FAIL_READ_DATA;
			seq_printf(s, "enter digital test failed!\n");
			err_cnt++;
			goto TEST_END;
		}
		...
		if (nvt_enter_digital_test(chip_info, false)) {
			TPD_INFO("exit digital test failed!\n");
			digital_noise_p_result = -NVT_MP_FAIL_READ_DATA;
			digital_noise_n_result = -NVT_MP_FAIL_READ_DATA;
			seq_printf(s, "exit digital test failed!\n");
			err_cnt++;
			goto TEST_END;
		}
...
static int nvt_tp_probe(struct spi_device *client)
{
	...
	/* 1. alloc chip info */
	chip_info = kzalloc(sizeof(struct chip_data_nt36528), GFP_KERNEL);
...
MODULE_DESCRIPTION("Novatek nt36528 noflash Driver");
MODULE_LICENSE("GPL");
