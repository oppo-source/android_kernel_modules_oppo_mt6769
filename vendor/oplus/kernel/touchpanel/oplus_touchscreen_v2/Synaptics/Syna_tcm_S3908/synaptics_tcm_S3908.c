...
extern int syna_remote_device_S3908_destory(struct syna_tcm_data *tcm_info);
...
static int syna_tcm_probe(struct spi_device *spi)
{
	...
	if (rc) {
		...
		TPD_INFO("S3908 configurations for mtk_platform_name : %s\n", mtk_platform_name);
...
#endif
        { .compatible = "synaptics-s3908_spi", },
        { }
};
