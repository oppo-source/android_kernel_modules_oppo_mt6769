...
static int check_dns_package(struct sk_buff *skb, int is_ingress)
{
	...
	if (skb->data_len > MAX_DNS_PACKET_LEN) {
		return -1;
	}
	return 0;
}

static int parse_querys(char *start, u32 payload_len, int *querys_len,
			query_info_t *queryList,
			uint16_t query_size, int *match_count, int hook)
{
	int matchsize = 0;
	int offset = 0;
	int i = 0;
	...
	for (i = 0; i < query_size; i++) {
		...
		if (offset >= (payload_len - 4)) {
			LOGK(1, "offset overflow %d-%u", offset, payload_len);
			break;
		}
		urlLen = strlen(start + offset + 1);
		...
		do {
			if (tmpLen > (urlLen - len)) {
				...
				spin_unlock_bh(&s_dns_hook_lock);
				return -1;
			}
			len += tmpLen;
			tmpLen = *(tmpurl + len);
			...
			len += 1;
			LOGK(0, "tmlen:%d, len:%d", tmpLen, len);
		} while (tmpLen != 0);
		...
		for (j = 0; j < MAX_DNS_HOOK_SIZE; j++) {
			if (s_hook_addrs[j].inuse) {
				if (strcmp(tmpurl, s_hook_addrs[j].url) == 0) {
					...
					LOGK(1, "hook %d parse_querys match %d-%d-%s-%u", hook, i, j, tmpurl,
					     queryList->addr);
					queryList += 1;
					matchsize++;
					if (s_first_notify == 0) {
...
static struct sk_buff *gen_dns_rsp(struct sk_buff *skb, int query_len,
				   query_info_t *query, int match_count)
{
	...
	skb_rsp = skb_copy_expand(skb, skb_headroom(skb),
				  skb_tailroom(skb) + answer_len, GFP_ATOMIC);
	if (skb_rsp == NULL) {
		kfree(ans_buf);
	...
	udph_rsp->check = 0;
	leftpayload = ntohs(udph_rsp->len) - sizeof(struct udphdr) - sizeof(
			      dnshdr_t) - query_len - answer_len;
	LOGK(0, "udplen: %d, leftpayload value %d, queryLen:%d", ntohs(udph_rsp->len),
	     leftpayload, query_len);
	/* update answer */
	...
	if (leftpayload != 0) {
		...
		ret = skb_copy_bits(skb_rsp,
				    skb_transport_offset(skb_rsp) + sizeof(struct udphdr) + sizeof(
					    dnshdr_t) + query_len, left_buf, leftpayload);
		if (ret) {
		...
		ret = skb_store_bits(skb_rsp,
				     skb_transport_offset(skb_rsp) + sizeof(struct udphdr) + sizeof(
					     dnshdr_t) + query_len, ans_buf, answer_len);
		if (ret) {
		...
		ret = skb_store_bits(skb_rsp,
				     skb_transport_offset(skb_rsp) + sizeof(struct udphdr) + sizeof(
					     dnshdr_t) + query_len + answer_len, left_buf,
				     leftpayload);
		if (ret) {
	...
	kfree(ans_buf);
	if (skb_rsp->protocol == htons(ETH_P_IP)) {
		skb_rsp->csum = csum_tcpudp_nofold(iph->saddr, iph->daddr,
						   skb_rsp->len - ip_hdrlen(skb), IPPROTO_UDP, 0);
...
static int dns_hook_process_output(struct sk_buff *skb, int hook,
				   const struct nf_hook_state *state)
{
	...
	payload_len = skb_network_header_len(skb) + sizeof(struct udphdr);
	ret = parse_querys((char *)(dnshdr + 1), payload_len, &query_len, query_list,
			   query_count, &match_count, hook);
	if (ret) {
		kfree(query_list);
		return -1;
	}
...
static unsigned int oplus_dns_hook_output_hook(void *priv, struct sk_buff *skb,
		const struct nf_hook_state *state)
{
	if (dns_hook_process_output(skb, state->hook, state) == 0) {
		kfree_skb(skb);
		return NF_STOLEN;
	}
	return NF_ACCEPT;
...
static int add_dns_hook_url(struct nlattr *nla)
{
	...
	if (!is_modify) {
		if (firstnotsetindex != -1) {
			...
			s_dns_hook_count++;
			LOGK(1, "add url %s %u", s_hook_addrs[firstnotsetindex].url,
			     s_hook_addrs[firstnotsetindex].addr);
...
static int oplus_dns_hook_netlink_rcv_msg(struct sk_buff *skb,
		struct genl_info *info)
{
	...
	LOGK(0, "set s_user_pid=%u type=%u len=%u.", nlhdr->nlmsg_pid, nla->nla_type,
	     nla->nla_len);
	if (s_user_pid == 0) {
...
static int send_netlink_data(int type, char *data, int len)
{
	...
	ret = genl_msg_prepare_usr_msg(OPLUS_DNS_HOOK_CMD_NOTIFY, size, s_user_pid,
				       &skbuff);
	if (ret) {
		return ret;
	}
...
static void oplus_dns_hook_netlink_exit(void)
{
	genl_unregister_family(&oplus_dns_hook_genl_family);
}
...
{
	oplus_dns_hook_table_hdr = register_net_sysctl(&init_net, "net/oplus_dns_hook",
				   oplus_net_hook_sysctl_table);
...
static int __init oplus_dns_hook_init(void)
{
	...
	ret = nf_register_net_hooks(&init_net, oplus_dns_hook_netfilter_ops,
				    ARRAY_SIZE(oplus_dns_hook_netfilter_ops));
	if (ret < 0) {
...
static void __exit oplus_dns_hook_fini(void)
{
	...
	oplus_dns_hook_netlink_exit();
	nf_unregister_net_hooks(&init_net, oplus_dns_hook_netfilter_ops,
				ARRAY_SIZE(oplus_dns_hook_netfilter_ops));
