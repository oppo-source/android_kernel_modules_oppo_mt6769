...
static void prep_new_page_dup(struct page *page, unsigned int order, gfp_t gfp_flags,
...
typedef void (*prep_compound_page_t)(struct page *page, unsigned int order);
static prep_compound_page_t prep_compound_page_dup = NULL;
...
/* As prep_new_page is inlined, we duplicate one here */
static void __nocfi prep_new_page_dup(struct page *page, unsigned int order, gfp_t gfp_flags,
							unsigned int alloc_flags)
{
	post_alloc_hook_dup(page, order, gfp_flags);
...
static int ux_page_pool_init(void)
{
	...
	struct kprobe prep_compound_page_kp = {
	...
	if (ret) {
		pr_err("get post_alloc_hook addr from kprobe failed! ret=%d\n", ret);
		return ret;
	}
	...
	ret = register_kprobe(&prep_compound_page_kp);
	if (ret) {
		pr_err("get prep_compound_page addr from kprobe failed! ret=%d\n", ret);
		return ret;
	}
