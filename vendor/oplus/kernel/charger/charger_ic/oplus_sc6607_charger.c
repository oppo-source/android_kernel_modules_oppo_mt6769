...
static int oplus_sc6607_disable_otg(void)
{
	...
	oplus_sc6607_request_otg_off(BOOST_ON_OTG);
	if (ret > 0) {
...
static void sc6607_init_status_work(struct work_struct *work)
{
	...
	if (!pg || chip->oplus_chg_type != POWER_SUPPLY_TYPE_UNKNOWN)
		return;
...
void oplus_chg_set_camera_on(bool val)
{
	...
	chg_chip->camera_on = val;
	if (chg_chip->dual_charger_support || oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
	    oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY)
		oplus_chg_set_flash_led_status(chg_chip->camera_on);
