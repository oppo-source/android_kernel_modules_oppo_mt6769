...
extern bool oplus_chg_get_gsm_call_on(void);
#endif
...
void __attribute__((weak)) oplus_chg_set_fix_mode(bool val)
{
	return;
}
...
static ssize_t gsm_call_ongoing_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (oplus_chg_get_gsm_call_on() == true)
		oplus_chg_set_fix_mode(chip->gsm_call_ongoing);
#endif
	return count;
}
					 ...
					 size_t count)
{
	...
	if (val == 0) {
		...
		if (chip->unwakelock_chg == 1) {
			...
			if (!chip->otg_online && oplus_get_charging_enable() == false)
