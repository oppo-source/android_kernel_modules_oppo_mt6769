...
#include "AC102_P_7_A0007_dsc_video_mode_panel.h"
#endif
...
static int tianma_unprepare(struct drm_panel *panel)
{
	...
	pr_info("[TP] tp_gesture_enable_flag = %d, shutdown_flag = %d, esd_flag = %lu \n", tp_gesture_enable_notifier(0), shutdown_flag, esd_flag);
	if (1 == shutdown_flag) {
		ctx->reset_gpio =
...
static int tianma_probe(struct mipi_dsi_device *dsi)
{
	...
	if (IS_ERR(ctx->bias_pos)) {
		pr_info("[error]%s: cannot get bias-pos 1 %ld\n",
			__func__, PTR_ERR(ctx->bias_pos));
		return PTR_ERR(ctx->bias_pos);
	}
	...
	if (IS_ERR(ctx->bias_neg)) {
		pr_info("[error]%s: cannot get bias-neg 2 %ld\n",
			__func__, PTR_ERR(ctx->bias_neg));
		return PTR_ERR(ctx->bias_neg);
	}
